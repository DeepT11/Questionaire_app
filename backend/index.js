...
app.get("/own_all_questions", (req, res) => {
  const id = req.session.userid;
  pool.query("select * from posts where owner_user_id = $1 and post_type_id = 1;", [id], (err, responses) => {
    if (err) {
      console.log(err);
      res.send({ err: err });
    }
...
app.post('/updateVote', (req, res) => {
  ...
  pool.query("DELETE FROM votes WHERE user_id = $1 AND post_id = $2 AND vote_type_id = $3", [userid, postid, othertype], (err, result) => {
